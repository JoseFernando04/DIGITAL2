...
// Cambia estos datos por los de tu red WiFi (puede ser la zona WiFi de tu celular)
const char* ssid = "Galaxy JosFer";      // Tu SSID aquí
const char* password = "12345678";  // Tu contraseña aquí

WebServer server(80);

// Pines de sensores LDR (ajusta según tu conexión)
const int ldrPins[8] = {34, 35, 32, 33, 25, 26, 27, 14}; // Pines ADC válidos del ESP32
const int UMBRAL = 1500; // Ajusta este valor según tu circuito y luz ambiente

uint8_t parkingStatus[8] = {0, 1, 0, 1, 0, 0, 1, 0}; // Inicialización de ejemplo
...
void setup() {
  ...
  // Inicializa pines de sensores (opcional, para LDR no es necesario)
  // for (int i = 0; i < 8; i++) pinMode(ldrPins[i], INPUT);
...
void loop() {
  ...
  // Lee los sensores LDR y actualiza el estado de los parqueos
  for (int i = 0; i < 8; i++) {
    int valor = analogRead(ldrPins[i]);
    parkingStatus[i] = (valor < UMBRAL) ? 1 : 0; // 1 = ocupado, 0 = libre
    // Para calibrar, puedes imprimir los valores:
    // Serial.print("LDR "); Serial.print(i); Serial.print(": "); Serial.println(valor);
  }
}
